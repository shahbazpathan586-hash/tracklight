...
| GET | `/api/conversions?siteId=X&days=30` | Conversion breakdown, sources, pages and recent conversion events |
...
- Add to cart clicks from common ecommerce buttons
- Checkout start clicks
...
TrackLight now separates conversion types while keeping one simple headline number.

- **Lead forms:** form submissions are tracked automatically as `form_complete` and count as conversions.
- **Add to cart:** common Add to Cart / Add to Bag / Add to Basket buttons are tracked automatically as `add_to_cart` and count as conversions.
- **Purchases:** thank-you/order-received pages and `TrackLight.purchase(...)` events count as purchases and conversions.
- **Custom conversions:** `TrackLight.track('conversion', {...})` still works for anything custom.

The main dashboard conversion number is **converted sessions**. If the same visitor clicks Add to Cart twice in one session, it records two add-to-cart events but still counts as one converted session. The Conversions view shows both the session count and the event breakdown.
...
TrackLight.addToCart({ productId: 'sku_123', value: 49, currency: 'USD' });
TrackLight.purchase({ orderId: 'ord_123', value: 99, currency: 'USD' });
