...
async function trackEvent(payload, options = {}) {
  if (!options.batch) await db.read();
...
async function _read() { await db.read(); }
...
async function getStats(siteId, days) {
  ...
  const todayVisitors = new Set(
    events.filter(e => e.ts.startsWith(todayKey)).map(e => e.visitorId)
  ).size;
...
async function getHeatmapPages(siteId) {
  ...
  db.data.heatmap
    ...
    .forEach(h => {
      const u = (h.url || '').replace(/^https?:\/\/[^/]+/, '').split('?')[0].split('#')[0] || '/';
      map[u] = (map[u] || 0) + 1;
    });
