...
function isConversionEventType(type) {
  return ['conversion', 'form_complete', 'add_to_cart', 'purchase', 'lead'].includes(type);
}

function readValue(meta) {
  const raw = meta && (meta.value ?? meta.revenue ?? meta.price ?? meta.amount);
  const n = Number(raw);
  return Number.isFinite(n) ? n : 0;
}

function getConversionBreakdown(events, sessions) {
  const formCompletes = events.filter(e => e.type === 'form_complete');
  ...
  const customConversions = events.filter(e => e.type === 'conversion');
  const phoneClicks = events.filter(e => e.type === 'phone_click');
  const emailClicks = events.filter(e => e.type === 'email_click');
  const revenue = purchases.reduce((a, e) => a + readValue(e.meta), 0);
  return {
    convertedSessions: sessions.filter(s => s.converted).length,
    leadForms: formCompletes.length,
    ...
    customConversions: customConversions.length,
    events: formCompletes.length + addToCarts.length + purchases.length + customConversions.length
  };
}
...
function periodSummary(siteId, startMs, endMs = Date.now()) {
  ...
  const conversionBreakdown = getConversionBreakdown(events, sessions);
...
function upsertSession(payload, ts) {
  ...
  if (existing) {
    ...
    if (isConversionEventType(payload.type)) {
      existing.converted = true;
      existing.conversionTypes ||= [];
      if (!existing.conversionTypes.includes(payload.type)) existing.conversionTypes.push(payload.type);
    }
    ...
    db.data.sessions.push({
      ...
      duration: payload.duration || 0,
      scrollDepth: payload.scrollDepth || 0,
      converted: isConversionEventType(payload.type),
      conversionTypes: isConversionEventType(payload.type) ? [payload.type] : [],
...
async function getStats(siteId, days) {
  ...
  return {
    ...
    funnel: {
      ...
      formCompletes,
      conversions,
      addToCarts: conversionBreakdown.addToCarts,
      checkoutStarts: conversionBreakdown.checkoutStarts,
      purchases: conversionBreakdown.purchases
    },
    conversionBreakdown,
    topCountries,
...
async function getConversionAnalytics(siteId, days = 30) {
  await db.read();
  ensureCollections();
  const now = Date.now();
  const since = now - (days || 30) * 86400000;
  ...
  const conversionEvents = events.filter(e => ['form_complete', 'add_to_cart', 'checkout_start', 'purchase', 'conversion', 'phone_click', 'email_click'].includes(e.type));
  const breakdown = getConversionBreakdown(events, sessions);

  const typeLabels = {
    form_complete: 'Lead form',
  ...
  conversionEvents.forEach(e => {
    ...
    typeMap[label] ||= { type: e.type, label, count: 0, revenue: 0 };
    typeMap[label].count++;
    typeMap[label].revenue += readValue(e.meta);
  });

  const pageMap = {};
  conversionEvents.forEach(e => {
    const page = e.page || normalizePage(e.url);
    pageMap[page] ||= { page, count: 0, revenue: 0 };
    pageMap[page].count++;
    pageMap[page].revenue += readValue(e.meta);
  });

  const sourceMap = {};
  sessions.filter(s => s.converted).forEach(s => {
    const source = classifySource(s.referrer);
    sourceMap[source] ||= { source, convertedSessions: 0 };
    sourceMap[source].convertedSessions++;
  });
  ...
  conversionEvents.forEach(e => {
    const day = (e.ts || '').split('T')[0];
    if (!day) return;
    dailyMap[day] ||= { date: day, leadForms: 0, addToCarts: 0, purchases: 0, customConversions: 0 };
    if (e.type === 'form_complete') dailyMap[day].leadForms++;
    else if (e.type === 'add_to_cart') dailyMap[day].addToCarts++;
    else if (e.type === 'purchase') dailyMap[day].purchases++;
    else if (e.type === 'conversion') dailyMap[day].customConversions++;
  });
  const trend = [];
  for (let i = (days || 30) - 1; i >= 0; i--) {
    const d = new Date(); d.setDate(d.getDate() - i);
    const key = d.toISOString().split('T')[0];
    trend.push(dailyMap[key] || { date: key, leadForms: 0, addToCarts: 0, purchases: 0, customConversions: 0 });
  }
  ...
  return {
    ...
    periodDays: days || 30,
    summary: breakdown,
    byType: Object.values(typeMap).map(r => ({ ...r, revenue: +r.revenue.toFixed(2) })).sort((a, b) => b.count - a.count),
    topPages: Object.values(pageMap).map(r => ({ ...r, revenue: +r.revenue.toFixed(2) })).sort((a, b) => b.count - a.count).slice(0, 10),
    bySource: Object.values(sourceMap).sort((a, b) => b.convertedSessions - a.convertedSessions),
    trend,
    recent: conversionEvents.slice().sort((a, b) => new Date(b.ts) - new Date(a.ts)).slice(0, 25).map(e => ({
      id: e.id,
      type: e.type,
      label: typeLabels[e.type] || e.type,
      page: e.page || normalizePage(e.url),
      ts: e.ts,
      meta: e.meta || {}
    }))
  };
...
async function getClientReport(siteId, days = 30) {
  const [stats, health, ai, funnel, movers, onboarding, conversions] = await Promise.all([
    ...
    getConversionAnalytics(siteId, days)
  ]);
  return { siteId, periodDays: days || 30, stats, health, ai, funnel, movers, onboarding, conversions };
}
...
module.exports = {
  ...
  getConversionAnalytics, getClientReport,
  _read, _write
};
